...
def test_basic_list():
    ...
    re = requests.get(config.url + 'channels/list/v2', params={'token': user_token['token']})
    ch_dict = re.json()
    assert len(ch_dict['channels']) == 1
    
#check when the list is empty 
def test_empty_list_v2():
    #reset route
    requests.delete(config.url + 'clear/v1')
...
def test_multiple_channels_and_users_v2():
    #reset route
    requests.delete(config.url + 'clear/v1')
...
#reset route
    ...
    requests.post(config.url + 'channels/create/v2',json={'token': user_token['token'],'name': 'Fox' , 'is_public' : True})
    re = requests.get(config.url + 'channels/list/v2', params={'token': invalid_token})
